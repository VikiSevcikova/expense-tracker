    ...
    try {
      ...
      if (transaction.transactionType === "" || transaction.categoryName === "" || transaction.amount === 0 || transaction.divideBy == 0 || transaction.paymentMethod === "" || isNaN(transaction.divideBy) || isNaN(transaction.amount)) {
        ...
        props.operationType === "edit" ?
          ...
          response = await axios.post(`${process.env.REACT_APP_SERVER_URL}/alltransaction/update/${props.checkedItem._id}`, tran, getHeaderConfig(token)) :
          //send data to backend - add new
          response = await axios.post(`${process.env.REACT_APP_SERVER_URL}/alltransaction/add`, tran, getHeaderConfig(token));
